...
my %extractFields = (
	nxttrftime => sub {
		...
		my $status    = Helpers::getCrontab($_[3], $_[4], '{settings}{status}');
		my $frequency = Helpers::getCrontab($_[3], $_[4], '{settings}{frequency}');
	...
	txt_mpc => sub {
		...
		$_[1]->{'content'}{'user_settings'}{'BACKUPLOCATION'} = (Helpers::getBackupDeviceID() . '#' . $d);
	},
	...
	lst_fullexclude => sub {
		...
		$_[1]->{'content'}{'settings'}{'fullExclude'} = parseExcludeFiles($d, $Configuration::fullExcludeListFile);
	},
	...
	lst_partexclude => sub {
		...
		$_[1]->{'content'}{'settings'}{'partialExclude'} = parseExcludeFiles($d, $Configuration::partialExcludeListFile);
	},
	...
	lst_regexexclude => sub {
		...
		$_[1]->{'content'}{'settings'}{'regexExclude'} = parseExcludeFiles($d, $Configuration::regexExcludeListFile);
	},
...
#			next if ($_[0]->{$_[1]}{$_}{'settings'}{'status'} eq 'disabled');
			...
			$sd{'bksetname'} = sprintf("%sBackupSet_%s",
				(($_ eq 'default_backupset') ? 'Default ' : 'Local'), Helpers::getMachineUser());
...
sub parseFilenames {
	...
	foreach (split(/^/, $_[0])) {
		...
		$_ =~ s/\n//g;
		if ($_ =~ /\\0$/) {
			$filename = $_[1] ? Helpers::urlDecode(substr($_, 0, -2)) : parseQuote(substr($_, 0, -2));
			next if ($filename =~ /^\s*$/);
			...
			$filename = $_[1] ? Helpers::urlDecode(substr($_, 0, -1)) : parseQuote(substr($_, 0, -1));
			next if ($filename =~ /^\s*$/);
			if (utf8::is_utf8($filename)) {
				utf8::downgrade($filename);
			}
...
sub parseExcludeFiles {
	my $pbkpf = Helpers::getCatfile(Helpers::getUserProfilePath(), $_[1]);
	my $partialFilenames = '';
	my $fileInfo = {};
	my $oldFileInfo = {};
...
#print "empty\n";
			return parseExcludeFiles($_[0], $Configuration::partialExcludeListFile);
		}
