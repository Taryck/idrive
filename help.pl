...
#wrapTextAndDisplay("Backup Location:\nUse this option to update the backup location.\nBackup Type:\nUse this option to change the backup type from mirror to relative and vice versa.");
#wrapTextAndDisplay("E-mail address:\n".$tab."Modify the email address provided at the time of setting up your account setup locally using this option.\nIgnore file/folder level permission error:\n".$tab."If your backup set contains files/folders that have insufficient access rights, $Configuration::appType will not backup those files/folders. In such a case, your backup will be considered as 'Failure' by default. Enable this setting to ignore file/folder level access rights/permission errors.\nProxy details:");
...
sub init {
	...
	while(1) {
		...
		my $userMainChoice= Helpers::getUserMenuChoice(scalar $Help{'heading'}->Keys);
		...
		if ($userMainChoice >= 0) {
			...
			if (defined($Help{'sub_heading'}{$mainHeading})) {
				if (defined($Help{'description'}->FETCH($mainHeading))) {
					Helpers::displayTitlewithUnderline($Help{'heading'}->FETCH($mainHeading));
					wrapTextAndDisplay($Help{'description'}->FETCH($mainHeading));
					wrapTextAndDisplay($Help{'description'}->FETCH($mainHeading."_".$Configuration::appType)) if($Help{'description'}->FETCH($mainHeading."_".$Configuration::appType));
				}
				...
				while(1) {
					...
					my $userChoice = Helpers::getAndValidate('enter_your_choice',"help_menu",1,1,scalar $Help{'sub_heading'}{$mainHeading}->Keys);
					...
					if ($Help{'description'}->FETCH($sub->[1][$userChoice])) {
						Helpers::displayTitlewithUnderline($Help{'heading'}->FETCH($mainHeading)." -> ".$sub->FETCH($sub->[1][$userChoice]));
						wrapTextAndDisplay($Help{'description'}->FETCH($sub->[1][$userChoice]));
						wrapTextAndDisplay($Help{'description'}->FETCH($sub->[1][$userChoice]."_".$Configuration::appType)) if($Help{'description'}->FETCH($sub->[1][$userChoice]."_".$Configuration::appType));

						Helpers::display(["\n",'press_keys_to_go_back_or_exit']);
						my $userChoice = Helpers::getAndValidate('enter_your_choice',"help_menu",1,1);
				...
				my $keyword = Helpers::getAndValidate(['enter_keyword',': '], 'non_empty', 1);
...
sub displaySearchResult {
	...
	my $appType = ($Configuration::appType eq 'IDrive')?'IBackup':'IDrive';
	foreach($_[0]->Keys) {
	...
	if (scalar($_[0]->Keys)) {
		foreach($_[0]->Keys) {
			Helpers::display($tableHeader) if($displayCount == 1);
			$tableContent 	.= $displayCount;
			# (total_space - used_space by data) will be used to keep separation between 2 data
			$tableContent 	.= qq( ) x ($columnNames[1]->[$spaceIndex] - length($displayCount));
			$spaceIndex++;
			...
			if ($displayCount%20 == 0 and $displayCount < scalar($_[0]->Keys)) {
				Helpers::display($tableContent);
				Helpers::display(["\n",'press_keys_load_more_or_to_go_back_or_exit']);
				$userChoice = Helpers::getAndValidate('enter_your_choice',"help_search_menu",1,1,$displayCount);
		...
		if(!defined($userChoice)) {
			if($tableContent ne '') {
				Helpers::display($tableContent);
				Helpers::display(["\n",'enter_the_serial_no_press_keys_to_go_back_or_exit']);
				$userChoice = Helpers::getAndValidate('enter_your_choice',"help_menu",1,1,scalar $_[0]->Keys);				
			} else {
				Helpers::display(["\n", 'no_help_found', ' ', 'please_try_again', '.']);
				Helpers::display(["\n",'press_keys_to_go_back_or_exit']);
				$userChoice = Helpers::getAndValidate('enter_your_choice',"help_menu",1,1);
			}
...
RESULT:
		$userChoice = $userChoice-1;
		if ( $_[0]->[1][$userChoice] =~ /$Configuration::appType/ and $Help{'description'}->FETCH($_[0]->[1][$userChoice])) {
			my $key 	= $_[0]->[1][$userChoice];
			my $mainKey = $_[0]->[1][$userChoice];
			$appType = "_".$Configuration::appType;
			$mainKey =~ s/$appType//;
			...
			wrapTextAndDisplay($Help{'description'}->FETCH($_[0]->[1][$userChoice]."_".$Configuration::appType)) if($Help{'description'}->FETCH($_[0]->[1][$userChoice]."_".$Configuration::appType));
		} else {
			Helpers::display(["\n", 'no_help_found', ' ', 'please_try_again', '.']);
		}
...
sub searchByKeyword {
	...
	#my $skipAppType = ($Configuration::appType eq 'IDrive')?'IBackup':'IDrive';
	foreach($desc->Keys) {
...
sub findHeadingsAndDisplay {
	...
	if($selectedKey =~ /$Configuration::appType$/) {
		$selectedKey =~ s/_$Configuration::appType$//;
	}
	...
	Helpers::displayTitlewithUnderline($header);
}
...
sub displayGoBackOrExitOption {
	Helpers::display(["\n",'press_keys_to_go_back_or_exit']);
	my $continueMenu = Helpers::getAndValidate(['enter_your_choice'], "PE_choice", 1);
